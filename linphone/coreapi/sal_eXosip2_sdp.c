...
#else
		int ptnumber = payload_type_get_number(pt);
		if (ptnumber == 0)
		{
			pt->mime_type = ms_strdup("PCMU");
			pt->clock_rate = 8000;
		}
		else if (ptnumber == 8)
		{
			pt->mime_type = ms_strdup("PCMA");
			pt->clock_rate = 8000;
		}
		else if (ptnumber == 18)
		{
			pt->mime_type = ms_strdup("G729");
			pt->clock_rate = 8000;
		}
		else
		{
...
{
	sdp_message_t *sdp_msg = eXosip_get_sdp_info_from_str(sdp);
	if (sdp_msg == NULL)
	{
		return -1;
	}

	return sdp_to_media_description(sdp_msg, desc);
}